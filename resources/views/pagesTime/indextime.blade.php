...
<div class="col-lg-12">
        <div class="p-5">
            <div class="text-center">
                <h1 class="h4 text-gray-900 mb-4">Chọn phòng học</h1>
            </div>
            <form action="{{route('findschedule')}}" method="POST" class="user">
            @csrf
                      ...
                      @case(1)
					  		<li  class="single-event" data-start="09:00" data-end="10:30" >
								<a type="button" class="btn btn-primary" data-toggle="modal" data-target="#M{{$row->pivot->id}}" >
									<em class="event-name">{{$row->classn[0]->classname}}</em>
								</a>
							</li>
                              @break
                          @case(2)
                          ...
                          <li  class="single-event" data-start="15:30" data-end="17:00" >
								<a type="button" class="btn btn-primary" data-toggle="modal" data-target="#M{{$row->pivot->id}}" >
									<em class="event-name">{{$row->classn[0]->classname}}<br></em>
								</a>
							</li>
                              @break
                          @case(5)
                    ...
                    @foreach($class->time as $row)
                      ...
                      <div class="modal fade" id="M{{$row->pivot->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                          <div class="modal-dialog modal-dialog-centered" role="document">
                            ...
                            <form action="{{route('destroytime',[$row->pivot->id, $row->room[0]->id])}}" method="post">
                              <a href = "{{ route('edittime', [$row->pivot->id, $row->room[0]->id]) }}"><button type="button" class="btn btn-primary">Thay đổi</button><a> </a>
                              @csrf
                              @method('DELETE')
                              <a onclick="return confirm('Bạn có chắc muốn hủy lịch trình này, bạn vẫn muốn tiếp tục ?')">
                              <button type="submit" class="btn btn-danger">Hủy lịch</button></a>
                            </form>
